...
industry are rich with legal entities and their functions. These document are very long and have information present in 
both structured and  unstructured format. Business wants to know what kind of a customer document is this 
...
existing technology (e.g. retraining of models based on proprietary data-sets, development of new variants of 
...
it. In order to leverage transfer learning, we can create our own dataset and start off by 
fine-tuning Spacy. Spacy underneath the hood uses a CNN based neural architecture with a few tweaks. Lately, Spacy 3.0 was 
...
<p align="justify">
Since address extraction is much more custom and complex in nature, we can use a BERT 
...
concept of cities and states in them. We can use Libpostal to parse these address spans or 
use a gazetteer for this. </p> <br>
...
<p align="justify">
We can curate a ternary relation extraction dataset amongst organizations, person names and roles form these docuemnts. 
We can then use a BERT based backbone with a relation head on top of it to solve this problem. An important part of this 
task is to decide which specific token embedding should we forward to the relation head. There are many 
techniques involved, people use max pooling or average all the token embeddings that represents the entities
and feed that to the relation layer. To make the network aware of the presence of specific tokens, we can also use entity
markers around the Organizations and Roles which can help us to achieve better results. </p> <br>
